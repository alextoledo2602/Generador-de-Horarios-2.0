...
export function EditTeacher() {
  ...
  useEffect(() => {
    teachersApi.get(id)
      ...
      .catch((err) => {
        setError("No se pudo cargar el profesor. " + (err.response?.status === 401 ? "No autorizado" : ""));
        setName("");
      });
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await teachersApi.update(id, { name });
  ...
  return (
    <div className="w-full py-10 px-4 md:px-10 flex justify-center">
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-xl border-2 border-white p-8 mx-auto">
        <h1 className="text-2xl font-bold text-[#006599] mb-2">Editar Profesor</h1>
        <p className="text-gray-600 mb-8">Modificar los datos del profesor</p>
        {error && (
