...
import { facultiesApi } from "../api/tasks.api";
...
export function EditFaculty() {
  ...
  useEffect(() => {
    facultiesApi.get(id)
      ...
      .catch(() => setError("No se pudo cargar la facultad."));
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await facultiesApi.update(id, { name });
      ...
      let msg = "Error al actualizar la facultad.";
  ...
  return (
    <div className="w-full py-10 px-4 md:px-10 flex justify-center">
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-xl border-2 border-white p-8 mx-auto">
        <h1 className="text-2xl font-bold text-[#006599] mb-2">Editar Facultad</h1>
        <p className="text-gray-600 mb-8">Modificar los datos de la facultad</p>
        {error && (
