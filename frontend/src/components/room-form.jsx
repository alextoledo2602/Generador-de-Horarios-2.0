...
export function RoomForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await class_roomsApi.create({ name });
      navigate("/locales");
    } catch (err) {
      let msg = "Error al crear el local.";
  ...
  return (
    <div className="w-full py-10 px-4 md:px-10 flex justify-center">
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-xl border-2 border-white p-8 mx-auto">
        <h1 className="text-2xl font-bold text-[#006599] mb-2">Añadir Nuevo Local</h1>
        <p className="text-gray-600 mb-8">Crear un nuevo local</p>
        {error && (
