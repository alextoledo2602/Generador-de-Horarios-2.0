...
export function EditSubject() {
  ...
  useEffect(() => {
    async function fetchSubject() {
      try {
        ...
        // Buscar la carrera y facultad asociada
        const careerRes = await careersApi.get(subject.career);
        const career = careerRes.data;
        setFormData({
          name: subject.name,
          symbology: subject.symbology,
          hours_found: subject.hours_found,
          type: subject.type,
          faculty_id: career.faculty,
          career_id: subject.career,
          year_id: subject.year,
        ...
        setSubmitError("Error al cargar la asignatura.");
      }
    }
    fetchSubject();
  }, [id, teachers.length]);
  ...
  useEffect(() => {
    ...
    careersApi.getAll().then((response) => {
      setCareers(
  ...
  // Cargar años cuando cambia la carrera
  useEffect(() => {
    if (!formData.career_id) {
      setYears([]);
      return;
    }
    yearsApi.getAll().then((response) => {
      setYears(
        response.data.filter(
  ...
  const handleTeacherSearch = (e) => setSearchTerm(e.target.value);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let msg = "Error al actualizar la asignatura.";
  ...
  return (
    <div className="container mx-auto py-10 px-4 md:px-10">
      <div className="w-full max-w-7xl bg-white rounded-xl shadow-xl border-2 border-white p-8 mx-auto">
        <h1 className="text-2xl font-bold text-[#006599] mb-2">Editar Asignatura</h1>
        <p className="text-gray-600 mb-8">Modificar los datos de la asignatura</p>
        {submitError && (
